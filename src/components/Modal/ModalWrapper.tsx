...
export default function ModalWrapper({setShow,productId}:Props) {
  const [details,setDetails] = useState({name:'',gender:'',description:'',price:0,discount:0})
  ...
  const getData = async()=>{
    try{
      ...
      const {result }= data
      console.log(data) 
      setDetails({description:result.description,gender:result.gender,name:result.name,price:result.price,discount:result.offer})
      setSize(result.size)
     ...
     <section className=' z-50 fixed top-0 bottom-0 right-0 left-0   flex justify-center items-center overflow-hidden' style={{height:"100vh",width:"100vw",backgroundColor:'rgb(253 253 253 / 86%)'}}>
            <div className='w-9/12 h-5/6 bg-white rounded border'>
              <div className='w-full flex flex-row-reverse pr-5 '><button onClick={closeModal} className='border-black m-3 font-thin text-6xl'>&times;</button></div>
                ...
                <button className='py-2 px-8 border mx-5 bg-green-400 rounded hover:bg-green-500'>Update</button>
                  <button  className='py-2 px-8 border bg-red-500 rounded hover:bg-red-600'>Delete</button>
                    ...
                    <div className='w-full h-full flex'>
                        ...
                        <div className='h-3/5 w-full '>
                           <div className='flex flex-wrap'>
                            ...
                            <br />
                            <input type="text" className='px-2 py-1 outline-none border border-slate-300 rounded' value={details.discount}/>
                            </div>
                            <div className='mx-2'>
                            <label className='p-1 font-bold text-zinc-700 text-sm'>Discount Price</label>
                            <br />
                            <input type="text" className='px-2 py-1 outline-none border bg-white border-slate-300 rounded' disabled value={ details.price - (details.price * details.discount)}/>
                            </div>
                           </div>
                           ...
                           <div className='flex flex-wrap  border border-slate-200 mr-3'>
                                ...
                                <div className='w-24 h-16 p-1 border border-dashed bg-zinc-100 rounded m-2'> <button className='w-full h-full  text-sm text-violet-700'>Add more +</button> </div>                            
                           </div>
                        </div>  
                        </div>     
